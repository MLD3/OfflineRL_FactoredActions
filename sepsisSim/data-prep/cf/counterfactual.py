...
class MatrixMDP(object):
    ...
    def policyEval(self, pi, discount=0.9, theta=1e-12):
        # Define the marginalized transition and reward matrix
        r_mat_obs = self.r_mat.T.dot(self.p_mixture).T
        tx_mat_obs = self.tx_mat.T.dot(self.p_mixture).T
        
        nS = self.tx_mat.shape[-2]
        nA = self.tx_mat.shape[-3]
        P = tx_mat_obs.transpose((1,0,2))
        R = np.zeros((nS, nA))
        gamma = discount
        for s in range(nS):
            for a in range(nA):
                R[s,a] = tx_mat_obs[a, s, :] @ r_mat_obs[a, s, :]
...
def policy_eval_analytic(env, pi, gamma):
    """
    Given the MDP model (transition probability P and reward function R),
